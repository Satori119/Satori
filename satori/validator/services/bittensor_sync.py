...
from satori.common.utils.retry import retry_sync_with_backoff
...
class BittensorSyncService:
    ...
    def get_all_miners(self):
        try:
            return self.client.get_all_miners()
        except Exception as e:
            logger.error(f"Failed to get all miners: {e}", exc_info=True)
            return []
    
    def set_weights(self, uids: list, weights: list):
        try:
            self.client.set_weights(uids, weights)
        except Exception as e:
