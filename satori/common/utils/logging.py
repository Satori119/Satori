...
def reinitialize_all_loggers(log_file: Optional[str] = None):
    ...
    satori_logger_names = [name for name in logging.Logger.manager.loggerDict.keys() if name.startswith('satori')]

    reinitialized_count = 0
    for name in satori_logger_names:
        logger = logging.getLogger(name)
        logger.setLevel(level)
    ...
    root_logger = logging.getLogger('satori')
    ...
    print(f"[logging] Reinitialized {reinitialized_count} satori loggers: {satori_logger_names}", flush=True)
